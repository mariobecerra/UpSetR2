...
upset2 <- function(data, 
                   ...
                   text.scale = 1, 
                   set_size.angles = 0 , 
                   set_size_x_axis.angles = 0, 
                   set_size.show = FALSE, 
                   set_size.numbers_size = NULL, 
                   set_size.scale_max = NULL,
                   sets.comma = F, 
                   mainbar.comma = F, 
                   intersection.size.comma = F, 
                   set_size.number_comma = F){
  ...
  if(type_of_data == "all"){
    first.col <- 1
    last.col <- ncol(data)
    Set_names = names(data)[1:last.col]
  ...
  } else if(type_of_data == "counts") {
    first.col <- 1
    last.col <- ncol(data) - 1
    Set_names = names(data)[1:last.col]
    
    message("Using count data.")
  } else stop('type_of_data must be either "all" or "counts"')
  
  
  All_Freqs = data %>%
    set_names(c(Set_names, "freq")) %>%
    arrange(desc(freq))
  
  Set_sizes = data.frame(y = 1:last.col, x = Set_names, stringsAsFactors = F)
  
  for(i in 1:last.col){
    Set_sizes$y[i] = sum(All_Freqs[,i]*All_Freqs$freq)
  }
  ...
  Main_bar <- suppressMessages(
    Make_main_bar(All_Freqs, Bar_Q, show.numbers, mb.ratio, customQBar, number.angles, EBar_data, mainbar.y.label,
  ...
  Sizes <- Make_size_plot(Set_sizes, sets.bar.color, mb.ratio, sets.x.label, scale.sets, 
                          text.scale, set_size.angles, set_size_x_axis.angles, set_size.show,
